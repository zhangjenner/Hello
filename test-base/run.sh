...
function generateChannelArtifacts() {
  ...
  echo
  echo "##########################################################"
  echo "### Generating channel configtx 'ChannelAB.tx' ###########"
  echo "##########################################################"
  ...
  echo "##########################################################"
  for MSP in BankAMSP BankBMSP; do
    configtxgen -profile ChannelAB -outputAnchorPeersUpdate ./channel-artifacts/${MSP}anchors.tx -channelID channel-ab -asOrg ${MSP}
    if [ "$?" -ne 0 ]; then
